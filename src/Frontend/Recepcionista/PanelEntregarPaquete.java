 ...
 */
public class PanelEntregarPaquete extends javax.swing.JPanel {
    ...
    public void obtenerPaquetes(int tipo){
        if(tipo == 0){
            listadoPaquetes = manejadorDB.obtenerListadoPaquetes("SELECT A.Codigo, A.Peso, A.Destino, A.DPICliente, A.PrecioTotal FROM Paquete A, PaqueteAsignadoRuta B WHERE B.EnDestino = TRUE && Entregado = FALSE "
                                                                        + "&& A.Codigo = B.CodigoPaquete LIMIT 45;");
        }
        else{
            listadoPaquetes = manejadorDB.obtenerListadoPaquetes("SELECT A.Codigo, A.Peso, A.Destino, A.DPICliente , A.PrecioTotal FROM Paquete A, PaqueteAsignadoRuta B WHERE B.EnDestino = TRUE && Entregado = FALSE "
    ...
    */
    public void establecerFiltroDeBusqueda(){
        ...
        if(radioBotonDPI.isSelected()){    
            listadoPaquetes = this.manejadorBusqueda.busquedaPaquetePorDPI(patronBusqueda, 0);
        }
        if(radioBotonCodigo.isSelected()){
            listadoPaquetes = this.manejadorBusqueda.busquedaPaquetePorCodigo(patronBusqueda, 0);
        }
        if(radioBotonDestino.isSelected()){
            listadoPaquetes = this.manejadorBusqueda.busquedaPaquetePorDestino(patronBusqueda, 0);
        }
        this.llenarTabla(listadoPaquetes);
    }
   ...
   public void inicializarCasillasSeleccion(){
    ...
    private void initComponents() {
        ...
        botonEntregar = new rojerusan.RSButtonIconI();
        ...
        botonEntregar.setBackground(new java.awt.Color(0, 153, 255));
        botonEntregar.setForeground(new java.awt.Color(0, 102, 153));
        botonEntregar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Frontend/Imagenes/caja.png"))); // NOI18N
        botonEntregar.setText("Entregar");
        botonEntregar.setColorHover(new java.awt.Color(153, 153, 153));
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addGap(13, 13, 13)
                .addComponent(botonEntregar, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        );
    ...
    /*
    Metodo encargado de registrar la entrega de un paquete. Valida que se encuentre seleccionado un paquete
    y obtien el valor seleccionado, lo almacena en la instancia paquete y manda a guardar la entrega 
    llamando al metodo entregarPaquete. Por ultimo lanza un mensaje informativo y refresca el listado de 
    paquetes.
    */
    private void botonEntregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonEntregarActionPerformed
        if(tablaPaquete.getSelectedRow() == -1){
            ...
            mensaje = manejadorDB.entregarPaquete(paquete);
    ...
    */
    ...
    private rojerusan.RSButtonIconI botonEntregar;
